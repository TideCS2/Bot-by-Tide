...
REDIRECT_URI = os.getenv("REDIRECT_URI") or "https://your-railway-app.up.railway.app/callback"
...
async def add_channel(interaction: discord.Interaction, twitch_username: str, role: discord.Role):
    ...
    await db.commit()
    await interaction.followup.send(f"✅ Added {twitch_username} → {role.name}")
...
async def assign_roles(discord_id, twitch_access_token):
    ...
    for guild_id, channel, broadcaster_id, role_id in channels:
        try:
            is_following = await check_follow(twitch_user_id, broadcaster_id, twitch_access_token)
            ...
            elif not is_following and role in member.roles:
...
async def callback(request):
    ...
    if not code or not state:
        return web.Response(text="Missing code or state")
    ...
    return web.Response(text="✅ Verified! Your Discord roles have been updated. You can close this tab.")
